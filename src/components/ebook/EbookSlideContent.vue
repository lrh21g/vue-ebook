    ...
    doSearch (q) {
      return Promise.all(
        this.currentBook.spine.spineItems.map(
          section => section.load(this.currentBook.load.bind(this.currentBook)) // section.load - 加载资源
            .then(section.find.bind(section, q)) // section.find - 从章节中查找字符串
            .finally(section.unload.bind(section))) // 将section原型释放，减少内存占用
      ).then(results => {
        // results - 为二维数组
        // [].concat.apply([], results) - 将二维数组降维，扁平化
        // apply - 会将数组变为参数进行传递
